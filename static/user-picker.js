(function(global) {
  ...
  function createUserPicker(options) {
    ...
    const title = normalizeText(config.title) || '选择用户';
    const url = normalizeText(config.url) || './view/uum/userPicker.html';
    const width = toLayerSize(config.width || 960) || '960px';
    const height = toLayerSize(config.height || 640) || '640px';
    let seed = 0;
  ...
  global.MyCuckoo = Object.assign((global.MyCuckoo || {}), {createUserPicker: createUserPicker});
