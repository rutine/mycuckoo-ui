...
layui.define(["jquery","laytpl"], function (exports) {
  ...
  Class.prototype={
    ...
    config: {
      ...
      contextmenuList: [],        //1 启用右键菜单，支持的操作有："copy","add","dialog","remove"
    ...
    rightClickMenu: function() {
      ...
      var menuStr=['<ul id="tree-menu">'
        ...
        ,$.inArray("dialog",options.contextmenuList)!==-1?'<li class="dialog"><a href="javascript:;">修改</a></li>':''
      ...
      options.elem.off("contextmenu").on("contextmenu",".eleTree-node-content",function(e) {
        ...
        $("#tree-menu li.copy,#tree-menu li.add,#tree-menu li.dialog,#tree-menu li.remove").show();
        $("#tree-menu").css({
          left: e.pageX,
          top: e.pageY
        }).show();
        ...
        $("#tree-menu li.add").off().on("click",function(e) {
          e.stopPropagation();
          $(this).hide().siblings("li.copy,li.dialog,li.remove").hide();
        ...
        // 编辑
        $("#tree-menu li.dialog").off().on("click",function(e) {
          e.stopPropagation();
          $("#tree-menu").hide().remove();
          var node=$(that).parent(".eleTree-node");
          var key=Number(node.attr("data-"+options.request.key));
