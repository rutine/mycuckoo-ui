...
layui.config({
  ...
  var $ = layui.$
    ...
    ,ELEM_VIEW = 'layui-table-view', ELEM_HEADER = '.layui-table-header', ELEM_BODY = '.layui-table-body', ELEM_MAIN = '.layui-table-main', ELEM_FIXED = '.layui-table-fixed', ELEM_FIXL = '.layui-table-fixed-l', ELEM_FIXR = '.layui-table-fixed-r', ELEM_TOOL = '.layui-table-tool', ELEM_PAGE = '.layui-table-page', ELEM_SORT = '.layui-table-sort', ELEM_EDIT = 'layui-table-dialog', ELEM_HOVER = 'layui-table-hover'
  ...
  Class.prototype.resetDatas=function(list) {
    ...
    if(options.isTree){//æ ç¶
      tableList=[];
      table.setDataList(that.config.id,tableList);
      var rootList=table.cache[options.id].data.upIds||[];//æ ¹èç¹listéå
      if(rootList.length<=0||that.resetDataresetRoot){//ç¡®å®æ ¹èç¹
        table.cache[options.id].data.upIds=[];
        rootList=that.resetDataRoot(list);
        that.resetDataresetRoot=false;
      }
      treeList=that.resetDataTreeList(list,rootList);
      table.setDataTreeList(that.config.id,treeList);//è®¾ç½®æ ç»æå°ç¼å­
      tableList=that.resetDataTableList(treeList);
      table.setDataList(that.config.id,tableList);//è®¾ç½®æ°æ®åè¡¨ç»æå°ç¼å­
    }
    //console.timeEnd("resetDatas");
    return datas;
  }
  /**
   * æ ¹æ®idä»è¡¨æ ¼æ°æ®ä¸­è·åå¯¹è±¡
   * @param data
   * @param field_Id
   * @param field_upId
   * @returns {Array}
  ...
  Class.prototype.renderTd=function (obj,cols,numbers,i3) {
    ...
    var td = ['<td data-field="'+ field +'" '+ function(){
      var attr = [];
      if(cols.dialog) attr.push('data-dialog="'+ cols.dialog +'"'); //是否允许单元格编辑
  ...
  Class.prototype.eventsinit = function(){
    ...
    that.layTool.on("click", "*[lay-event]", function (e) {
      var othis = $(this),
        c = othis.attr("lay-event");

      layui.event.call(this, MOD_NAME, 'toolbar(' + filter + ')', $.extend({event: c, config: options}, {}))
    });
    ...
    that.layBody.on('change','.'+ELEM_EDIT, function(){
      ...
      layui.event.call(this, MOD_NAME, 'dialog('+ filter +')', {
        value: value
        ,data: data
        ,field: field
      });
    ...
    that.layBody.on('blur','.'+ELEM_EDIT, function(){//单元格失去焦点
      var templet
        ...
        ,field = othis.parent().data('field')
        ,index = othis.parents('tr').eq(0).data('index')
        ,editType = othis.parent().data('dialog')
        ,data = table.getDataList(that.key)[index];
      var  options = that.config;
