  ...
  methods: {
    ...
    enable() {
      ...
      if(item.status == 'enable') {
        MyCuckoo.msg({ state: 'info', title : '提示', msg : '此操作已经启用' });
        return;
      }

      $vue.api.operateMgr.disEnable({id: item.operateId, disEnableFlag: 'enable'}).then(data => {
        MyCuckoo.msg({state: 'success', title: '提示', msg: '操作启用成功'});
    ...
    disable() {
      ...
      if(item.status == 'disable') {
        MyCuckoo.msg({ state: 'info', title : '提示', msg : '此操作已经停用' });
        return;
      }
      ...
      MyCuckoo.confirm({
        ...
        cancelBtn: '否',
        ok : function() {
          $vue.api.operateMgr.disEnable({id: item.operateId, disEnableFlag: 'disable'}).then(data => {
            MyCuckoo.msg({state: 'success', title: '提示', msg: '操作停用成功'});
